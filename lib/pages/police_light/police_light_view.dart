...
class PoliceLightView extends GetView<PoliceLightLogic> {
  const PoliceLightView({super.key});

  @override
  Widget build(BuildContext context) {
    return FullscreenLightPage(
      title: Lang.policeLight,
      onBack: controller.goBack,
      lightContent: Obx(() {
        final isOn = controller.isOn.value;
        final isRed = controller.isRed.value;
        return Container(
          width: double.infinity,
          height: double.infinity,
          color: isOn ? (isRed ? Colors.red : Colors.blue) : Colors.black,
        );
      }),
      controlPanel: Column(
